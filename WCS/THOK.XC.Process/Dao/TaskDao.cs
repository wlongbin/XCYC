    ...
    public  class TaskDao : BaseDao
    {
        ...
        {            
            ...
            if (dt.Rows[0]["CELL_CODE"].ToString() == "")
            {
                ...
                //{
                    ...
                    //再次判断这个货位是否已用到，如用到再次申请一次
                //    strSQL = string.Format("SELECT 1 FROM CMD_CELL WHERE CELL_CODE='{0}' AND IS_ACTIVE='1' AND IS_LOCK='0' AND PRODUCT_CODE IS NULL AND PRODUCT_BARCODE IS NULL AND BILL_NO IS NULL", VCell);
                //    dt = ExecuteQuery(strSQL).Tables[0];
                //    if (dt.Rows.Count > 0)
                //        VCell = "";
                //    i++;
        ...
        public bool PermissionOutToStation(string TaskNo)
        {
            ...
            string strSQL = string.Format("SELECT * FROM WCS_TASK_DETAIL DETAIL " +
                                          "LEFT JOIN WCS_TASK TASK ON DETAIL.TASK_ID=TASK.TASK_ID " +
                                          "WHERE DETAIL.TASK_NO='{0}' AND DETAIL.STATE IN('1','2','3') AND DETAIL.ITEM_NO=3 ", TaskNo);
            DataTable dt = ExecuteQuery(strSQL).Tables[0];
            if (dt.Rows.Count > 0)
            {
